...
- Docker (for production) or Python 3.8+ (for local development)
- Telegram Bot Token from [[https://t.me/botfather][@BotFather]]

** Installation

*** Quick Deploy with Pre-built Image (Recommended)

Deploy in seconds using pre-built images from GitHub Container Registry:
...
- Pulls latest image from =ghcr.io/mandoo180/telegram-note-bot=
...
- =/version= - Show bot version (useful to verify deployment)
...
2. Click *"📝 Edit Note"* button to open Web App editor
3. Fill in:
   - Title
...
├── webapp/              # Web App editor files
├── docs/                # Documentation
├── data/                # Database storage (gitignored)
├── init_db.py           # Database initialization
├── check_reminders.py   # Diagnostic tool
